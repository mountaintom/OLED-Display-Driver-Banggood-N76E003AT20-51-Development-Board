 ...
 **************************************************************************************************/
 ...
 /* The orginal Arduino libaray from rinkyDink is modified for low end controllers with small RAM memory.
    ExploreEmbedded oled_spi_i2c.c library does not have a RAM buffer and hence different fonts and graphics are not supported.
...
// __code const uint8_t OledFontTable[][FONT_SIZE]= // May try this
//-- const uint8_t xOledFontTable[]=
{
...
void oledSendCommand(uint8_t cmd)
{
// todo: Set cmd/data to cmd
    P13 = LOW; // D/C
    oledSendByte(cmd);
// todo Set cmd/data to data
    P13 = HIGH; // D/C
}
#endif
